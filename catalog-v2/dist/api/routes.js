...
function createCatalogRouter(options) {
    ...
    router.get('/products/recommendations', async (req, res) => {
        try {
            const productId = req.query.productId || req.query.product_id || req.query.id;
    ...
    router.get('/collections/:handle/products', async (req, res) => {
        try {
            ...
            for (let batchStart = 0; batchStart < handles.length; batchStart += 50) {
                ...
                const productsByHandle = new Map(fetchedProducts.filter(p => p && isVisibleProduct(p)).map(p => [p.handle, p]));
...
async function loadMixMetaIndex(cache) {
    ...
    return products.map((product) => ({
        ...
        collectionHandles: Array.isArray(product.collectionHandles) && product.collectionHandles.length
            ...
            : (product.collections?.edges || []).map((edge) => safeString(edge?.node?.handle)).filter(Boolean),
...
async function loadMixedCatalogProductsPage(cache, { mixKey, limit, after, inStockOnly = false }) {
    ...
    while (scanIndex < orderedHandles.length && pageProducts.length < pageLimit) {
        ...
        if (!batchHandles.length)
            break;
        const fetchedProducts = await fetchProductsByHandles(cache, batchHandles);
        const productsByHandle = new Map(fetchedProducts.filter(isVisibleProduct).map((p) => [p.handle, p]));
        for (const handle of batchHandles) {
            scanIndex++;
...
function buildCollectionProductsPayload(collection, pageProducts, pageInfo) {
    ...
    async function resolveCollectionProductHandles(cache, collection) {
        const storedHandles = (collection?.productHandles || [])
            .map((h) => safeString(h)).filter(Boolean);
    ...
    function buildCollectionProductHandlesIndex(products) {
        ...
        for (const product of products) {
            ...
            if (!productHandle || !isVisibleProduct(product))
                continue;
    ...
    function buildCollectionProductsPayload(collection, pageProducts, pageInfo) {
        ...
        async function tryCollectionProductsShopifyFallback(res, { handle, first, after, reason }) {
            ...
            const lookupHandles = Array.from(new Set([requestedHandle, resolveCanonicalCollectionHandle(requestedHandle)].filter(Boolean)));
        ...
        function safeGetCollection(cache, handle) {
            const requestedHandle = safeString(handle);
